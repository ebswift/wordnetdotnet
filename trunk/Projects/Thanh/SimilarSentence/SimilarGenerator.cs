...
{
	/// <summary>
	/// Summary description for SimilarGenerator.
	/// </summary>
	public class SimilarGenerator
	{
		...
		public SimilarGenerator(MyPos[] pos, string originalSentence)
		{
			_myPos=pos;
		...
		public MyPos[] FindSynonyms(ref MyPos pos, bool includeMorphs)
		{
		...
		class CompareLexeme : IComparer
		{
			...
			{
				return ((MyPos)y).Frequency - ((MyPos)x).Frequency;
			}
		}

		private MyPos[] LookupCandidates(Wnlib.Index index, MyPos pos )
		{						
			...
			for( int i=0; i < words.Length; i++ )
			{
				...
				MyPos newpos=new MyPos(word, pos.Pos) ;
			...
			return (MyPos[])candidates.ToArray( typeof(MyPos) );
		}

		
		private int GetaNeighbour(MyPos[] current, out MyPos[] trial)
		{			
			trial=(MyPos[])current.Clone() ;
		...
		private int InitialSentence(out MyPos[] current)
		{
			current=new MyPos[_myPos.Length] ;
			int overall=0;
		...
		private void IterativeGenerate(int trialNum, float descentFactor)
		{
			...
			{								
				...
				{
					MyPos[] trial;
					int new_cost=GetaNeighbour (current,out trial);
		...
		{				
			_alterWord=new MyPos[_myPos.Length][] ;
			...
			{		
				...
				if (pos.Pos != PartsOfSpeech.Unknown && pos.Sense != -1)
				{
					...
					{
						foreach(MyPos poss in _alterWord[i])						
							poss.Pos=pos.Pos;
		...
		{
			++_numItems ;
			MyPos[] pos=new MyPos[_myPos.Length] ;
			string newsen="";
			...
			{									
				...
				words[i]=word;
				pos[i]=new MyPos(word, _myPos[i].Pos) ;
			}
		...
		private void MyInit()
		{
			...
			_contextWords=new MyPos[_myPos.Length] ;
