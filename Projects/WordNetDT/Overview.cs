...
namespace WordNet
{
	...
	public class Overview
	{
		...
		// format lists
		private string decodeOther(string buf)
		{
			string tmpstr = null;
			string[] tmparr = null;
			string retstr = null;
			...
			for (i = 2; i <= tmparr.GetUpperBound(0); i++) {
				...
				if (senseflag) {
					...
					retstr += formatWrdDefn(wrdlst, defn);
					x += 1;
					senseflag = false;
				// this is an indented definition
					...
					if (Strings.InStr(tmpstr, "CATEGORY TERM") > 0) {
						...
						wrdlst = formatWrdDefn(wrdlst, defn);
		...
		private string formatWrdDefn(string wrdlst, string defn)
		{
			string retstr = null;

			retstr = formatWordList(wrdlst);

			defn = linkDefinition(defn);
			// turn definition words into hyperlinks
			if (!string.IsNullOrEmpty(defn)) {
				retstr += ":";
			}

			retstr += " <span class='Defn'>" + defn + "</span><br />" + Constants.vbCrLf;
