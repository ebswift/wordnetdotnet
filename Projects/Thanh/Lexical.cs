...
namespace WnLexicon
{
	...
	public class Lexicon
	{
		...
		private static string[] lookupSynonyms( Wnlib.Index  index )
		{
			...
			Opt op=Opt.at(13) ;
			Search se = new Search(index.wd, true, op.pos , op.sch, int.Parse("0"));//searchWord, ,PointerType's label,pos,scense
			// for each sense...
			for( int s=0; s<index.offs.Length; s++ )
			{
				// read in the word and its pointers
				Wnlib.SynSet synset = new Wnlib.SynSet( index.offs[s], index.pos, index.wd, se, s );
